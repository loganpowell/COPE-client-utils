...
const nodeUpdate = ({ id, new_id, type, status, owner, createdAt }, authMode = GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS) => __awaiter(void 0, void 0, void 0, function* () {
    if (!new_id) {
    ...
    const edges = yield getConnectedNodesByNodeID({ id });
    const new_edges = edges && (yield Promise.all(edges.map(({ edge }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const todos = yield Promise.all(ass.items.map(({ id, }) => __awaiter(void 0, void 0, void 0, function* () {
        return yield op.update({ id, node_id: new_id });
    })));
